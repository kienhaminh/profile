...
describe('Blog API Contract Tests', () => {
  ...
  beforeAll(async () => {
    // Clean up any existing test data first (delete in correct order due to foreign keys)
    // First, find and delete any posts by the test author
    const testAuthors = await db
      .select()
      .from(authorProfiles)
    ...
    if (testAuthors.length > 0) {
      ...
      await db
        ...
        .where(eq(authorProfiles.id, testAuthorId));
    }

    await db.delete(posts).where(eq(posts.slug, 'test-blog-unique'));
    await db.delete(posts).where(eq(posts.slug, 'test-blog-update'));
    await db.delete(topics).where(eq(topics.slug, 'test-topic'));
    await db.delete(hashtags).where(eq(hashtags.slug, 'test-hashtag'));
