...
describe('Filtering and Search Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    // Setup topics - use unique names to avoid conflicts
    const [topicJS] = await db
      .insert(topics)
      .values({
        name: 'JavaScript Filter Test',
        slug: 'javascript-filter-test',
      })
      .returning();
    ...
    const [topicTS] = await db
      .insert(topics)
      .values({
        name: 'TypeScript Filter Test',
        slug: 'typescript-filter-test',
      })
      .returning();
    topicTypeScriptId = topicTS.id;

    // Setup hashtags - use unique names to avoid conflicts
    const [hashtagTut] = await db
      .insert(hashtags)
      .values({ name: 'tutorial filter test', slug: 'tutorial-filter-test' })
      .returning();
    hashtagTutorialId = hashtagTut.id;

    const [hashtagAdv] = await db
      .insert(hashtags)
      .values({ name: 'advanced filter test', slug: 'advanced-filter-test' })
      .returning();
    hashtagAdvancedId = hashtagAdv.id;

    // Setup technologies - use unique names to avoid conflicts
    const [techR] = await db
      .insert(technologies)
      .values({ name: 'React Filter Test', slug: 'react-filter-test' })
      .returning();
    techReactId = techR.id;

    const [techN] = await db
      .insert(technologies)
      .values({ name: 'Node.js Filter Test', slug: 'nodejs-filter-test' })
      .returning();
    techNodeId = techN.id;
