...
This workspace script wraps `mint dev --port 3001` so the preview avoids clashing with the Next.js app.

A local preview of your documentation will be available at `http://localhost:3001`.
...
By default, Mintlify uses port 3000. You can customize the port Mintlify runs on by using the `--port` flag. For example, the workspace script above binds the server to 3001, and you can choose any other port with:
